	...
	Vue.prototype.page = function(page, page_num, low_page, more_page, little_page, mid_page, start_page, end_page) {
		...
		if (page_num <= low_page) {
			...
			if (page < more_page) {
				...
				page_arr.push('...');
				for (var n = page_num * 1 - little_page * 1 + 1; n <= page_num; n++) {
					page_arr.push(n);
				}
				return page_arr;
			} else if (page >= more_page && page <= page_num * 1 - more_page * 1) {
				for (var n = 1; n <= start_page; n++) {
					page_arr.push(n);
				}
				page_arr.push('...');
				for (var n = page * 1 - Math.floor(mid_page / 2); n <= page * 1 + Math.ceil(mid_page / 2) - 1; n++) {
					page_arr.push(n);
				}
				page_arr.push('...');
				for (var n = page_num * 1 - end_page * 1 + 1; n <= page_num; n++) {
					page_arr.push(n);
				}
				return page_arr;
			} else {
				...
				page_arr.push('...');
				for (var n = page_num * 1 - more_page * 1 + 1; n <= page_num; n++) {
					page_arr.push(n);
				}
				return page_arr;
			}
